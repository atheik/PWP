...
def _get_image(synset_wnid="n02103406", imid=9, url="http://farm3.static.flickr.com/2056/2203156496_bf1b977326.jpg", date=None):
    return Image(
        synset_wnid=synset_wnid,
        imid=imid,
        url=url,
        date=date
    )
...
def test_create_instances(app):
    ...
    with app.app_context():
        synset = _get_synset()
        synset_image = _get_image()
        synset_hyponym = _get_synset(wnid="n02109047", words="Great Dane", gloss="very large powerful smooth-coated breed of dog")
        synset_hyponym_image = _get_image(synset_wnid="n02109047", imid=11, url="http://farm1.static.flickr.com/123/403783566_7a838f13c2.jpg")
...
def test_image_ondelete_synset(app):
    ...
    with app.app_context():
        ...
        # TODO This fails because synset_image is loaded into memory?
        # TODO The DELETE methods work fine...
        db_synset = Synset.query.filter_by(wnid="n02103406").first()
